...
void cSceneManager::LoadScene(GLuint shaderProgramID, GLFWwindow* window, std::vector <cMesh*> &MeshList, glm::vec3 mainCameraPosition, glm::vec3 mainCameraTarget, bool isFreeFlowCam, bool applyLOD, 
    bool drawColliders, std::vector <cMesh*>& ColliderMeshList)
{
    ...
    for (sSceneDetails* currentScene : this->mSceneDetailsList)
    {
        ...
        {
            // If the scene has more than one camera angles but since it's a main scene we are just gonna focus through one(1st camera angle is made default here)
            if (currentScene->bMainScene && camAngleCount > 0)
                continue;
            ...
            {
                if (MeshList[index]->alphaTransparency < 1.f)
                    currentScene->bSceneHasTransparencyMesh = true;

                if (MeshList[index]->sceneId == currentScene->sceneID) // Checking if Mesh is drawn this Scene
                {
            ...
            if (drawColliders)
            {
                for (unsigned int index = 0; index != ColliderMeshList.size(); index++)
                    this->DrawSceneObject(ColliderMeshList[index], glm::mat4(1.0f), shaderProgramID, width, height);
            }
...
void cSceneManager::SwitchMainScene(unsigned int sceneIdToSwitchTo)
{
    sSceneDetails* previousMainScene = this->FindScene(this->GetMainSceneId());
    sSceneDetails* currentScene = this->FindScene(sceneIdToSwitchTo);

    if (currentScene == NULL)
        return;
    else if (currentScene == previousMainScene)
        std::cout << "ERROR : GIVEN SCENE ID IS THE MAIN SCENE" << std::endl;

    previousMainScene->bMainScene = false;
    currentScene->bMainScene = true;

    if (currentScene->bTurnIntoFBOTexture)
    {
        currentScene->bTurnIntoFBOTexture = false;

        for (cFBO* currentFBO : currentScene->FBOList)
        {
            if (currentFBO->shutdown())
                std::cout << "ERROR : UNABLE TO SHUTDOWN FBO" << std::endl;

            delete currentFBO;
        }

        currentScene->FBOList.clear();
    }
}

unsigned int cSceneManager::GetMainSceneId()
{
    for (sSceneDetails* currentScene : this->mSceneDetailsList)
    {
        if (currentScene->bMainScene)
...
cSceneManager::sSceneDetails* cSceneManager::FindScene(unsigned int sceneId)
{
    if (sceneId >= this->mSceneDetailsList.size())
    {
    ...
    return this->mSceneDetailsList[sceneId];
}
