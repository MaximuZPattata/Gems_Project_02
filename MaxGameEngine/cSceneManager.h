...
public:
	...
	void LoadScene(GLuint shaderProgramID, GLFWwindow* window, std::vector <cMesh*>& MeshList, glm::vec3 mainCameraPosition, glm::vec3 mainCameraTarget, 
		bool isFreeFlowCam, bool applyLOD, bool drawColliders, std::vector <cMesh*>& ColliderMeshList);
	...
	void SwitchMainScene(unsigned int sceneIdToSwitchTo);
...
private:
	...
	sSceneDetails* FindScene(unsigned int sceneId);
	unsigned int GetMainSceneId();
};
